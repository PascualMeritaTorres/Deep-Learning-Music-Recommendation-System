...
def get_auc(est_array, gt_array):
    ...
    '''
    unique_gt_classes = np.unique(gt_array)
    
    if len(unique_gt_classes) <= 1:
        ...
        try:
            ...
            print("Error calculating ROC AUC score. Skipping ROC AUC calculation.")
            roc_aucs = -1
    
    try:
        pr_aucs = metrics.average_precision_score(gt_array, est_array, average='macro')
    except ValueError:
        print("Error calculating PR AUC score. Skipping PR AUC calculation.")
        pr_aucs = -1
...
def get_tensor(fn,data_path,input_length,batch_size,mode,augmentation_class):
    ...
    # load audio
    npy_path = os.path.join(data_path, 'mtat', 'npy', fn.split('/')[1][:-3]) + 'npy'
    raw = np.load(npy_path, mmap_mode='r').copy() 
...
def get_scores(mode,model,list_to_iterate_on,batch_size,binary,data_path,input_length, augmentation_class=DataAugmentation('time_stretch',0)):
    ...
    for line in tqdm.tqdm(list_to_iterate_on):
        ...
        # forward
        x=Variable(x)
        y = torch.tensor(np.array([ground_truth.astype('float32') for i in range(batch_size)]))  # Convert list of numpy.ndarrays to a single numpy.ndarray
        out = model(x)
